...
trait PureManifest {
  ...
  def seed(set: Set[_]): UUID = UUID.nameUUIDFromBytes(Array(java.lang.Byte.valueOf(set.size.toString)))
  def time(set: Set[_]): Instant = Instant.ofEpochMilli(1524870034204L).plusSeconds(set.size)
