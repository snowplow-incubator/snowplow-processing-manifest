...
object Serialization {
    ...
    if (map.isEmpty) { parseError("DynamoDB record does not contain any payload").asLeft } else {
      ...
      val appState: ValidatedNel[String, (Application, State, UUID)] =
        getS(AppStateKey).toValidatedNel.andThen(v => v.split(quote(Separator.toString), -1).toList match {
          case List(appName, appVersion, instanceId, step, uuid) =>
            (State.parse(step).toValidatedNel, parseId(uuid).toValidatedNel).mapN { (step, state) =>
              (Application(Agent(appName, appVersion), expand(instanceId)), step, state)
            }
          case _ =>
  ...
  private[dynamodb] def appState(app: Application, id: UUID, step: State): String =
    s"${showApplication(app)}$Separator${showState(id, step)}"
  ...
  def showState(id: UUID, step: State): String =
    s"${step.show}$Separator$id"
