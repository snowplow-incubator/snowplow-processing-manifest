...
/**
  ...
  */
abstract class ProcessingManifest[F[_]](resolver: Resolver)(implicit private[manifest] val F: ManifestAction[F]) {
    ...
    def getHandler(item: Item, created: Boolean): F[LockHandler[F]] =
      item.records.find(_.state == State.New).map(_.recordId) match {
        ...
        case Some(_) =>
          LockHandler.pure[F]
        case None =>
          val error = "'processNewItem' was called on item without new step"
